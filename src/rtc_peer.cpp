...
MediaFrame::MediaFrame(webrtccore::MediaType media_type, char *data, int32_t len,
                       uint64_t timestamp, uint32_t ssrc, uint32_t payload_type,
                       webrtccore::VideoRotation rotate_angle, uint32_t audio_frame_len) {
...
/*send do*/
void RtcPeer::OnAddLocalVideoTrack(
    ...
    while (itor != codec_info_map.end()) {
        ...
        coco_info("local worker_id[%d] payload_type:%d codec_type:%u profile_level_id:%s\r\n",
                  worker_id_, video_codec_info->payload_type, video_codec_info->codec_type,
...
/*send do*/
void RtcPeer::OnAddLocalAudioTrack(
    uint32_t ssrc, const std::string &label,
    ...
    while (itor != codec_info_map.end()) {
        ...
        coco_info(
            "local worker_id[%d], key %u,  payload_type:%d codec_type:%u sample_rate:%d "
            "channels:%d\r\n",
            worker_id_, itor->first, audio_codec_info->payload_type, audio_codec_info->codec_type,
            audio_codec_info->sample_rate, audio_codec_info->channels);
...
void RtcPeer::OnAddAudioTrack(uint32_t ssrc, const std::string &label,
    ...
    while (itor != codec_info_map.end()) {
        const webrtccore::AudioCodeInfo *audio_codec_info = &(itor->second);
        coco_info(
            "remote worker_id[%d], key %u, payload_type:%d codec_type:%u sample_rate:%d "
            "channels:%d\r\n",
            worker_id_, itor->first, audio_codec_info->payload_type, audio_codec_info->codec_type,
            audio_codec_info->sample_rate, audio_codec_info->channels);
        itor++;
...
void RtcPeer::OnAddVideoTrack(uint32_t ssrc, const std::string &label,
    ...
    while (itor != codec_info_map.end()) {
        const webrtccore::VideoCodeInfo *video_codec_info = &(itor->second);
        coco_info(
            "remote worker_id[%d] key %u, payload_type:%d codec_type:%u profile_level_id:%s\r\n",
            worker_id_, itor->first, video_codec_info->payload_type, video_codec_info->codec_type,
            video_codec_info->profile_level_id.c_str());
        itor++;
    }
...
void RtcPeer::OnDeliverRtpPacket(webrtccore::MediaType type,
                                 std::shared_ptr<webrtccore::RtpPacket> packet) {}
...
void RtcPeer::OnRequestIFrame(uint32_t ssrc) { coco_dbg("*** will request Iframe for %u", ssrc); }
...
webrtccore::PeerConnectionInterface *RtcPeer::GetPC() { return peer_connection_; }

void RtcPeer::SetPC(webrtccore::PeerConnectionInterface *pc) { peer_connection_ = pc; }

void RtcPeer::SetUdpFd(int32_t udp_fd) { udp_fd_ = udp_fd; }

void RtcPeer::SetDumpFile(FILE *dump_file) { fp_h264_ = dump_file; }

void RtcPeer::SetWorkerId(int32_t worker_id) { worker_id_ = worker_id; }

void RtcPeer::UdpSocketRecv(uint8_t *data, int32_t len, const webrtccore::NetAddr &remote_addr) {
...
void RtcPeer::GetMediaStats() {
    ...
    if (0 == ret) {
        out_len += snprintf((log_out_buffer_ + out_len), (LOG_BUFF_LEN - out_len), " Stm   %s\r\n",
        ...
        out_len += snprintf((log_out_buffer_ + out_len), (LOG_BUFF_LEN - out_len),
                            ...
                            pc_status.rtt, pc_status.send_bitrate_bps >> 10,
    ...
    if (local_video_ssrc_) {
        ...
        if (0 == ret) {
            ...
            out_len += snprintf((log_out_buffer_ + out_len), (LOG_BUFF_LEN - out_len),
                                ...
                                "lable:%s\r\n",
                                video_status.codec_info.codec_type, video_status.ssrc,
                                video_status.codec_info.payload_type,
                                video_status.codec_info.profile_level_id.c_str(),
                                video_status.lable.c_str());
            out_len +=
                snprintf((log_out_buffer_ + out_len), (LOG_BUFF_LEN - out_len),
                         " Net   rtt:%u bitrate:%" PRIu64 "->%" PRIu64 "kpbs\r\n", video_status.rtt,
                         video_status.core_bitrate_bps >> 10, video_status.real_bitrate_bps >> 10);
            out_len += snprintf((log_out_buffer_ + out_len), (LOG_BUFF_LEN - out_len),
        ...
        if (0 == ret) {
            ...
            out_len +=
                snprintf((log_out_buffer_ + out_len), (LOG_BUFF_LEN - out_len),
                         ...
                         " chn:%u lable:%s\r\n",
                         audio_status.codec_info.codec_type, audio_status.ssrc,
                         audio_status.codec_info.payload_type, audio_status.codec_info.sample_rate,
                         audio_status.codec_info.channels, audio_status.lable.c_str());
            out_len +=
                snprintf((log_out_buffer_ + out_len), (LOG_BUFF_LEN - out_len),
                         " Net   rtt:%u bitrate:%" PRIu64 "->%" PRIu64 "kpbs\r\n", audio_status.rtt,
                         audio_status.core_bitrate_bps >> 10, audio_status.real_bitrate_bps >> 10);
            out_len += snprintf((log_out_buffer_ + out_len), (LOG_BUFF_LEN - out_len),
    ...
    if (remote_video_ssrc_) {
        ...
        if (0 == ret) {
            ...
            out_len += snprintf((log_out_buffer_ + out_len), (LOG_BUFF_LEN - out_len),
                                ...
                                video_status.lable.c_str());
            out_len += snprintf((log_out_buffer_ + out_len), (LOG_BUFF_LEN - out_len),
                                " Net   rtt:%u bitrate:%" PRIu64 "->%" PRIu64
                                "kpbs "
                                "loss:%.2f%%->%.2f%%\r\n",
                                video_status.rtt, video_status.core_bitrate_bps >> 10,
            ...
            out_len += snprintf((log_out_buffer_ + out_len), (LOG_BUFF_LEN - out_len),
                                ...
                                video_status.recv_pkg_num, video_status.lost_pkg_num,
                                video_status.pli_cnt, video_status.recv_idr_num,
                                video_status.frozen_time, video_status.pop_frame_num);
        }
    }
    ...
    if (remote_audio_ssrc_) {
        ...
        if (0 == ret) {
            ...
            out_len += snprintf((log_out_buffer_ + out_len), (LOG_BUFF_LEN - out_len),
                                ...
                                "kpbs "
                                "loss:%.2f%%->%.2f%%\r\n",
                                audio_status.rtt, audio_status.core_bitrate_bps >> 10,
            ...
            out_len +=
                snprintf((log_out_buffer_ + out_len), (LOG_BUFF_LEN - out_len),
                         ...
                         audio_status.lost_pkg_num, audio_status.pop_frame_num);
        }
    }
