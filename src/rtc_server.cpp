    ...
    if (Stun::IsStun((uint8_t *)buf, len)) {
        ...
        if (type == Stun::Class::REQUEST && method == Stun::Method::BINDING) {
            ...
            if (connit == SarsPeersUfragMaps.end()) {
                coco_error("no remote peer username %s for candidate %s", username.c_str(),
                           remote.c_str());
                return;
            }
...
int32_t RTCServer::CreateRtcPeer(std::shared_ptr<RtcPeer> peer, const std::string stream_id,
                                 std::string &offer, std::string &answer, bool hasLocal) {
    ...
    webrtccore::PeerConnectionInterface *pc =
        peer_connection_factory->CreatePeerConnection(peer.get(), 1, stream_id, pc_init_param);
    ...
    webrtccore::RTCOfferAnswerOptions options(1, 1, webrtccore::kAddrIpv4, _lOuterAddr, _lport,
                                              true, webrtccore::kPassive);
    /*get sdp*/
    pc->CreateAnswer(answer, options);

    std::string ufrag;
    if (getUfragFromSdp(offer, ufrag) != COCO_SUCCESS) {
        coco_dbg("sdp must contain ufrag");
        return -1;
    }
    std::string lufrag;
    if (getUfragFromSdp(answer, lufrag) != COCO_SUCCESS) {
        coco_dbg("sdp must contain ufrag");
        return -1;
    }
...
std::shared_ptr<RtcPublisher> RTCServer::CreateRtcPublisher(const std::string stream_id,
                                                            std::string &offer,
                                                            std::string &answer) {
...
std::shared_ptr<RtcPlayer> RTCServer::CreateRtcPlayer(const std::string stream_id,
                                                      std::string &offer, std::string &answer) {
