...
const uint8_t Stun::magicCookie[] = {0x21, 0x12, 0xA4, 0x42 };


Stun* Stun::Parse(const uint8_t* data, size_t len)
{
    ...
    // Create a new Stun (data + 8 points to the received TransactionID field).
    auto packet = new Stun(
...
Stun::Authentication Stun::CheckAuthentication(
...
Stun* Stun::CreateSuccessResponse()
{
    //MS_TRACE();
    ...
    return new Stun(Class::SUCCESS_RESPONSE, this->method, this->transactionId, nullptr, 0);
}

Stun* Stun::CreateErrorResponse(uint16_t errorCode)
{

    auto response =
            new Stun(Class::ERROR_RESPONSE, this->method, this->transactionId, nullptr, 0);
...
void Stun::Authenticate(const std::string& password)
{
...
void Stun::Serialize(uint8_t* buffer)
{
    ...
    bool addXorMappedAddress =
            ((this->xorMappedAddress != nullptr) && this->method == Stun::Method::BINDING &&
    ...
    // Set magic cookie.
    std::memcpy(buffer + 4, Stun::magicCookie, 4);
        ...
        switch (this->xorMappedAddress->sa_family)
        {
            case AF_INET:
            {
                ...
                std::memcpy(
                        ...
                        2);
                attrValue[2] ^= Stun::magicCookie[0];
                attrValue[3] ^= Stun::magicCookie[1];
                // Set address and XOR it.
                std::memcpy(
                        attrValue + 4,
                        &(reinterpret_cast<const sockaddr_in*>(this->xorMappedAddress))->sin_addr.s_addr,
                        4);
                attrValue[4] ^= Stun::magicCookie[0];
                attrValue[5] ^= Stun::magicCookie[1];
                attrValue[6] ^= Stun::magicCookie[2];
                attrValue[7] ^= Stun::magicCookie[3];
